...
int8_t  brightness = 100; // in %
int8_t  dimLevel = 0;    // dim level.  0 = off
...
Adafruit_ILI9341 tft = Adafruit_ILI9341(TX, WKP, RX);
...
Button buttons[] =
{// Left, top, wid, hi
    {224, 137,  90, 25},    // 0 cool temp high (do not change positon: buttons 0-3 refrenced elsewhere)
    {224, 162,  90, 25},    // 1 cool temp low
    {224, 187,  90, 25},    // 2 heat temp high
    {224, 212,  90, 25},    // 3 heat temp low

    {224,  108,  90, 25},    // 4 target temp
...
void drawButton(int8_t btn, bool bDown, bool bFast) // draw a button with pressed state and updated text
{
    ...
    int16_t l = (buttons[btn].w>>1) - (tft.propStringLength(pszText)>>1);
    int16_t t = (buttons[btn].h>>1) - FONT_Y - 1;

    tft.setCursor(buttons[btn].x + l, buttons[btn].y + t);
    tft.setTextColor(fg, ILI9341_BLACK);
    int x = tft.drawPropString((char*)pszText, false);

    if(bDeg)
        tft.drawCircle(x + 1, buttons[btn].y + t + 3, 2, fg);     // add a degree symbol
    tft.drawRoundRect(buttons[btn].x, buttons[btn].y, buttons[btn].w, buttons[btn].h, 3, bg); // draw rect after text
}
...
void DrawButtons()
{
    ...
    for(i = 0; i < 3; i++)
    {
        tft.setCursor(buttons[i+8].x + buttons[i+8].w + 10, buttons[i+8].y + 7);
    	tft.drawPropString((char *)szLabels2[i], false);
    }
...
//		analogWrite(DAC1, 4096);
	...
	else //1...99%
	{
    	pinMode(DAC1, OUTPUT);
        digitalWrite(DAC1, HIGH);
//		analogWrite(DAC1, (uint16_t)bright*40);
	}
}
...
void drawSignal(int x, int y, int wh)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        tft.drawLine(x + i*dist, y, x + i*dist, y - (i+1)*dist+4, (sigStrength > i * sect) ? ILI9341_GREEN : GRAY2);
    }
}
...
void setup()
{
	...
	Particle.variable("settings", hvac.m_szSettings, STRING);
}
...
void loop()
{
	...
	if(Time.second() != lastSec)   // things to do every second
	{
        ...
        if(lastHour != Time.hour())     // things to do on the hour
        {
            ...
            UpdateEE();          // EE updates will go here for now (every hour)
	    }

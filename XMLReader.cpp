...
void XMLReader::sendHeader(const char *pHeaderName, const char *pHeaderValue)
{
...
void XMLReader::sendHeader(const char *pHeaderName, int nHeaderValue)
{
	m_client.print(pHeaderName);
	m_client.print(": ");
...
bool XMLReader::service(XML_tag_t *tags)
{
    ...
    do
    {
        ...
        if(m_pIn == m_pEnd) // buffer full
        {
            ...
            else if(!tags[m_tagIdx].pszTag)
            {
                m_Status = XML_DONE;
                m_client.stop();
                return false;  // all done
            }
        	else if( combTag(tags[m_tagIdx].pszTag, tags[m_tagIdx].pszAttr, tags[m_tagIdx].pszValue))
        	{
        		m_binValues = true;
        		m_valIdx = 0;
        	}
...
//    Serial.print("fillbuffer byte avail = ");
//    Serial.println( m_client.available() );
//    Serial.println( m_pIn - m_pBuffer );
...
{
	if(m_pPtr >= m_pEnd)	// remove all used bytes
    {
        m_pPtr = m_pBuffer;
        m_pIn = m_pBuffer;
    }
...
bool XMLReader::tagCompare(char *p1, const char *p2) // compare at lenngth of p2
{
	while(*p2)
	{
